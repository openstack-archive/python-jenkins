...
from six.moves.urllib.request import BaseHandler, build_opener
import kerberos

logger = logging.getLogger(__name__)


class HTTPNegotiateHandler(BaseHandler):
    ...
    def http_error_401(self, req, _, __, ___, headers):
        logger.debug("INSIDE http_error_401")
        try:
            try:
                krb_req = self._extract_krb_value(headers)
            except ValueError:
                # Negotiate header not found or a similiar error
    ...
    def _krb_response(self, host, krb_val):
        ...
        _, self.krb_context = kerberos.authGSSClientInit("HTTP@%s" % host)
...
def test():
    ...
    opener = build_opener()
    opener.add_handler(HTTPNegotiateHandler())
    resp = opener.open(sys.argv[1])
    print(resp.code)
    # print("".join(resp.readlines()))
