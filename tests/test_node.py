...
class JenkinsCreateNodeTest(JenkinsNodesTestBase):
    ...
    def test_urlencode(self, jenkins_mock):
        ...
        self.assertEqual(json_data['json']['launcher']['username'], 'juser')
        self.assertEqual(json_data['json']['launcher']['stapler-class'],
                         u'hudson.plugins.sshslaves.SSHLauncher')
        self.assertEqual(json_data['json']['launcher']['host'],
                         'my.jenkins.slave1')
        self.assertEqual(json_data['json']['remoteFS'], '/home/juser')
        self.assertEqual(json_data['json']['labelString'], 'precise')
        self.assertEqual(json_data['json']['name'], 'slave1')
        self._check_requests(jenkins_mock.call_args_list)
