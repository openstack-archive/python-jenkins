...
class JenkinsCreateNodeTest(JenkinsNodesTestBase):
    ...
    def test_urlencode(self, jenkins_mock):
        ...
        json_data = jenkins_mock.call_args_list[1][0][0].data
        self.assertEquals(json_data['json']['launcher']['username'], 'juser')
        self.assertEquals(json_data['json']['launcher']['stapler-class'],
                          u'hudson.plugins.sshslaves.SSHLauncher')
        self.assertEquals(json_data['json']['launcher']['host'],
                          'my.jenkins.slave1')
        self.assertEquals(json_data['json']['remoteFS'], '/home/juser')
        self.assertEquals(json_data['json']['labelString'], 'precise')
        self.assertEquals(json_data['json']['name'], 'slave1')
        self._check_requests(jenkins_mock.call_args_list)
