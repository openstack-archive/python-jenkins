...
class JenkinsInfoTest(JenkinsTestBase):
    ...
    @patch.object(jenkins.Jenkins, 'jenkins_open')
    def test_jenkins_host_prefix(self, jenkins_mock):
        job_info_to_return = {
            u'jobs': {
                u'url': u'http://your_url_here/job/my_job/',
                u'color': u'blue',
        ...
        prefixed_jenkins = jenkins.Jenkins(
            'http://example.com/prefix/', 'test', 'test')
        job_info = prefixed_jenkins.get_info()

        self.assertEqual(job_info, job_info_to_return)
        self.assertEqual(
            jenkins_mock.call_args[0][0].get_full_url(),
            u'http://example.com/prefix/api/json')
        self._check_requests(jenkins_mock.call_args_list)
