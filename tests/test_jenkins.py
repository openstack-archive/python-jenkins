...
class JenkinsTest(unittest.TestCase):
    ...
    @patch.object(jenkins.Jenkins, 'jenkins_open')
    def test_rename_view(self, jenkins_mock):
        jenkins_mock.side_effect = [
            json.dumps({'name': 'Test View_2'}),
            json.dumps({'name': 'Test View_2'}),
            json.dumps({'name': 'Test View_2'}),
        ]
        j = jenkins.Jenkins('http://example.com/', 'test', 'test')

        j.rename_view(u'Test View', u'Test View_2')

        self.assertEqual(
            jenkins_mock.call_args_list[0][0][0].get_full_url(),
            'http://example.com/view/Test%20View/doRename?newName=Test%20View_2')
        self.assertTrue(j.view_exists('Test View_2'))
        self._check_requests(jenkins_mock.call_args_list)

    @patch.object(jenkins.Jenkins, 'jenkins_open')
    def test_rename_view__rename_failed(self, jenkins_mock):
        jenkins_mock.side_effect = [
            None,
            jenkins.NotFoundException(),
        ]
        ...
        with self.assertRaises(jenkins.JenkinsException) as context_manager:
            j.rename_view(u'TestView', u'TestView_2')
        self.assertEqual(
            jenkins_mock.call_args_list[0][0][0].get_full_url(),
            'http://example.com/view/TestView/doRename?newName=TestView_2')
        self.assertEqual(
            str(context_manager.exception),
            'rename[TestView_2] failed')
        self._check_requests(jenkins_mock.call_args_list)
