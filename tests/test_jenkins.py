...
class JenkinsConstructorTest(JenkinsTestBase):
    ...
    def test_url_with_trailing_slash(self):
        self.assertEqual(self.j.server, self.makeUrl(''))
    ...
    def test_url_without_trailing_slash(self):
        ...
        self.assertEqual(j.server, self.makeUrl(''))
...
class JenkinsMaybeAddCrumbTest(JenkinsTestBase):
    ...
    def test_simple(self, jenkins_mock):
        ...
        request = jenkins.Request(self.makeUrl('job/TestJob'))

        self.j.maybe_add_crumb(request)

        self.assertEqual(
            jenkins_mock.call_args[0][0].get_full_url(),
            self.makeUrl('crumbIssuer/api/json'))
...
class JenkinsOpenTest(JenkinsTestBase):
    ...
    def test_timeout(self, jenkins_mock):
        jenkins_mock.side_effect = jenkins.URLError(
            reason="timed out")
        j = jenkins.Jenkins(self.makeUrl(''), 'test', 'test', timeout=1)
        request = jenkins.Request(self.makeUrl('job/TestJob'))
