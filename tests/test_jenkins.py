...
class JenkinsTest(unittest.TestCase):
    ...
    @patch.object(jenkins.Jenkins, 'jenkins_open')
    def test_get_jobs_folders_simple(self, jenkins_mock):
        jobs = [
            [
                {'name': 'my_job1'},
        ...
        j = jenkins.Jenkins('http://example.com/', 'test', 'test')
        response = build_jobs_list_responses(jobs, 'http://example.com/')
        jenkins_mock.side_effect = iter(response)

        jobs_info = j.get_jobs()
        self.assertEqual(4, len(jobs_info))
    ...
    @patch.object(jenkins.Jenkins, 'jenkins_open')
    def test__get_job_folder(self, jenkins_mock):
        j = jenkins.Jenkins('http://example.com/', 'test', 'test')

        folder, name = j._get_job_folder('my job')
        self.assertEqual(folder, '')
        self.assertEqual(name, 'my job')

        folder, name = j._get_job_folder('my folder/my job')
        self.assertEqual(folder, 'job/my folder/')
        self.assertEqual(name, 'my job')

        folder, name = j._get_job_folder('folder1/folder2/my job')
